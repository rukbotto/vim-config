...
" Remark:
"   It lexically highlights embedded mustaches (exclusively) in html file.
...
syn region mustacheScriptTemplate
    \ start=+<script [^>]*type *=[^>]*text/\(mustache\|x-handlebars-template\)[^>]*>+
    \ end=+</script>+me=s-1 keepend
    \ contains=mustacheInside,@htmlMustacheContainer,htmlTag,htmlEndTag,htmlTagName,htmlSpecialChar

let b:current_syntax = "htmlmustache"
